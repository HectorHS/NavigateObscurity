...
export function fLower(string: string): string {
...
export function getCountryCode(country: string): string {
    let countryCode = new Map<string, string>();
    ...
    let code: string = countryCode.get(country)!;
    if (!code) {
...
export function getCountryName(country: string): string {
    let countryName = new Map<string, string>();
...
export function createLegend(chart_points: any[], data: any[], parentContainer: string): void {
    ...
    let legends: Element[] = Array.from(document.querySelectorAll(parentContainer + ' .chart-legend'));
...
export function addSlider(parent: Element, name: string, min: number, max: number, value: number, width: number, onChange: any): void {
...
export function getIndexColor(indx: number): string {
    let indexColor = new Map<number, string>();
    ...
    let color: string = indexColor.get(indx)!;
    if (!color) {
        console.log("Color not found for index " + indx)
    }
    return color;
}
function tabClickHandler(parent:HTMLElement, element:Element) {
    if (!(element.classList.contains("tab-active"))) {
        ...
        for (let i in parentNod!.childNodes) {
            ...
            if (el.nodeName == "DIV") {
                (el as Element).classList.remove("tab-active");
            }
        }
        ...
        let questionsText:Element = parent.getElementsByClassName("dash-questions")[0]!;
        let obscurityText:Element = parent.getElementsByClassName("dash-obscurity")[0]!;
        let navigationText:Element = parent.getElementsByClassName("dash-navigation")[0]!;
...
export function addTabClickEvents (css:string):void{
    let parentElement:HTMLElement = document.getElementById(css)!;
