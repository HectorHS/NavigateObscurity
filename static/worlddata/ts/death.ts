...
let tabcss:string = "death_intro";
addTabClickEvents(tabcss);

let LifeMap = d3.csv(life_map_path)
    .then(function (data:any):void {
        // All filterable options
        const ALL_PARAMETERS:string[] = [...new Set(data.map((d: { Parameter: any; }) => d.Parameter))].sort() as string[];
        const ALL_YEARS:string[] = [...new Set(data.map((d: { Year: any; }) => d.Year))].sort() as string[];
        let all_locations:string[] = data.columns as string[];
        ...
        let initial_data:Worlddata.MapData[] = [];

        function get_data():Worlddata.MapData[] {
            let new_data:{country:string, value:number}[] = [];
            for (let row of data) {
        ...
        let filters:Element = document.querySelector(container_class + " .chart-filters")!;
...
let DeathDash = Promise.all([
    ...
    // All filterable options
    const ALL_LOCATIONS:string[] = [...new Set(files[1].map((d: { Location: string; }) => d.Location as string))].sort() as string[];
    const ALL_AGES:string[] = [...new Set(files[0].map((d: { age: string; }) => d.age))].sort() as string[];
    const ALL_MAIN_CAUSES:string[] = [...new Set(files[2].map((d: { cause: string; }) => d.cause))].sort() as string[];
    const PYRAMID_CATEGORIES:string[] = [
    ...
    let causeElement:Element = document.getElementById("death-dashboard-cause")!;
    let locationElement:Element = document.getElementById("death-dashboard-location")!;
    let ageElement:Element = document.getElementById("death-dashboard-age")!;
    let sexElement:Element = document.getElementById("death-dashboard-sex")!;
    let radioElements:HTMLCollection = document.getElementsByClassName('sex-radio')!;

    // setting reseting click events
    (document.getElementById("resetCause") as Element).addEventListener("click", resetCause);
    (document.getElementById("resetLocation") as Element).addEventListener("click", resetLocation);
    (document.getElementById("resetAge") as Element).addEventListener("click", resetAge);
    (document.getElementById("resetSex") as Element).addEventListener("click", resetSex);
    for (let i = 0; i < radioElements.length; i++) {
        (radioElements[i] as HTMLInputElement).onchange = sexRadioChange;
    }

    function sexRadioChange():void {
        sex = (document.querySelector('input[name="sex-radio"]:checked') as HTMLInputElement).value!;
        updateSex();
        treemap_data_update();
        bubbles_data_update();
        map_data_update();
    ...
    function get_treemap_data(): Worlddata.TreemapData[] {
        let new_data: Worlddata.TreemapData[] = [];
        ...
        if (cause == "All causes") {
            for (let caus of ALL_MAIN_CAUSES) {
                ...
                if (new_data.length > 0) {
                    let slice:Worlddata.Mapseries = { data: new_data, name: caus, type: 'map', colorIndex: getColor.get(caus)!, showInLegend: false }
            ...
            let series:Worlddata.Mapseries = { data: new_data, name: cause, type: 'map', colorIndex: 999 }
            new_series.push(series);
        }
        return new_series;
    }
    function get_pyramid_slice_data(sex:string):Worlddata.BarData[] {
        let zero = 0;
        let ten = 0;
        let twenty = 0;
        let thirty = 0;
        ...
        let new_data:Worlddata.BarData[] = [];
        for (let i = 0; i < dataPC.length; i++) {
            let dv:Worlddata.BarData = { y: dataPC[i], totalValue: data[i] }
            new_data.push(dv);
        }

        return new_data;
    }
    function treemap_data_update():void {
    ...
    function pyramid_data_update():void {
    ...
    function map_data_update(col?:string) {
    ...
    function get_bubbles_data():Worlddata.BubblesData[] {

        var new_data:Worlddata.BubblesData[] = [];
        ...
        plotOptions: {
            ...
            series: {
                events: {
                    ...
                    setRootNode: function (e:Worlddata.RootNodeEvent):void {
        ...
        tooltip: {
            formatter: function ():string {
                return '<strong>Aged from ' + this.x + '</strong><br/>' +
                    ...
                    'Female, total number of deaths: ' + numberFormatter((this.points![1].point as Worlddata.BarData).totalValue!) + '<br>' +
    ...
    function setMapTitle():void {
        let element:HTMLElement = document.getElementById("death_map_title")!;
        let text:string;
    ...
    function setPyramidTitle():void {
        let element:HTMLElement = document.getElementById("death_pyramid_title")!;
    ...
    function setBubblesTitle():void {
        let element:HTMLElement = document.getElementById("death_bubbles_title")!;
        let text:string;
    ...
    function setTreemapTitle():void {
        let element:HTMLElement = document.getElementById("death_treemap_title")!;
        let text:string;
        if (cause == "All causes") {
    ...
    function resetSex():void {
        sex = "Both";
        sexElement.innerHTML = sex;
        for (var i = 0; i < radioElements.length; i++) {
            let elem = radioElements[i] as HTMLInputElement;
            if (elem.value! == "Both") {
                elem.checked = true;
            } else {
                elem.checked = false;
    ...
    function updateSex():void {
        let maleEl:Element = document.querySelector(".death-pyramid .highcharts-legend .highcharts-series-0 text")!;
        let maleSeriesEl:Element = document.querySelector(".death-pyramid  .highcharts-series-group .highcharts-series-0")!;
        let femaleEl:Element = document.querySelector(".death-pyramid .highcharts-legend .highcharts-series-1 text")!;
        let femaleSeriesEl:Element = document.querySelector(".death-pyramid  .highcharts-series-group .highcharts-series-1")!;
        sexElement.innerHTML = sex;
...
var oldDeathTree = d3.csv(old_death_path)
    .then(function (data:any[]):void {
        function get_data():Worlddata.TreemapData[] {
            var new_data:Worlddata.TreemapData[] = [];
            for (let row of data) {
...
let getMapColor = new Map<string, number>();
...
let getCauseParent = new Map<string, string>();
...
let getCauseMaxValue = new Map<string, number>();
