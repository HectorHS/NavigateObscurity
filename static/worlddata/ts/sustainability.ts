...
var sustainabilityMap = d3.csv(sustain_map_path)
    .then(function (data:any):void {
        ...
        const ALL_MEASURES:string[] = [...new Set(data.map((d: {Measure:string}) => d.Measure))].sort() as string[];
        const ALL_AREAS:string[] = [...new Set(data.map((d: {Area: string}) => d.Area))].sort() as string[];
        let all_inco:string[] = [...new Set(data.map((d: {IncomeGroup:string}) => d.IncomeGroup))].sort() as string[];
        ...
        function areaChange():void {
            area = (<HTMLSelectElement>document.getElementById(areaName + "-select")).value;
            updateColorAxis();
            chart.series[0].setData(get_data());
        }
        ...
        let getBiocapacityAverage = new Map<string, string>();
        ...
        let getReserveAverage = new Map<string, string>();
        ...
        let getConsumptionAverage = new Map<string, string>();
...
let sustainabilityArea = d3.csv(sustain_area_path)
    .then(function (data:any) {
        // All filterable options
        const ALL_MEASURES:string[] = [...new Set(data.map((d: {record:string}) => d.record))].sort() as string[];
        const ALL_COUNTRIES:string[] = [...new Set(data.map((d: {country: string}) => d.country))].sort() as string[];
        let all_years:string[] = data.columns as string[];
        ...
        function updateData():void {
            let loops:number = chart.series.length;
