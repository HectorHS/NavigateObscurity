...
let tabcss:string = "energy_intro";
...
var energyDashboard = Promise.all([
    ...
    let all_years:string[] = files[1].columns as string[];
    all_years.splice(0,2);
   
    let initial_area_data:Worlddata.AreaSeries[] = get_area_data();
    let initial_import_data:Worlddata.BarSeries[] = getTradeData("Import");
    let initial_import_xaxis:string[] = getTradeXAxis("Import");
    let initial_export_data:Worlddata.BarSeries[] = getTradeData("Export");
    let initial_export_xaxis:string[] = getTradeXAxis("Export");
    let initial_data:Worlddata.MapData[] = get_map_data();
    let initial_sector_data:Worlddata.SunburstData[] = getSectorData();

    let statEnergy:HTMLElement = document.getElementById("stat_energy")!;
    let statCO2:HTMLElement = document.getElementById("stat_co2")!;
    let statPop:HTMLElement = document.getElementById("stat_population")!;
    let statImport:HTMLElement = document.getElementById("stat_import")!;
    let statExport:HTMLElement = document.getElementById("stat_export")!;

    // radio
    document.getElementsByName('source-radio').forEach(function (a:any) {
        a.onchange = energyRadioChange;
    });

    function energyRadioChange():void {
        source = (<HTMLInputElement>document.querySelector('input[name="source-radio"]:checked')).value;
        // update area
        setAreaData();
        // update map
        updateColorStops();
        map_chart.series[0].setData(get_map_data());
        let title:HTMLElement = document.getElementById('map_title')!;
        let newTitle:string;
        if (source == "All") {
    ...
    function getSectorData():Worlddata.SunburstData[] {
        let newData:Worlddata.SunburstData[] = [];
    ...
    function get_area_data():Worlddata.AreaSeries[] {
        var new_data:Worlddata.AreaSeries[] = [];
    ...
    function getTradeData(type:string):Worlddata.BarSeries[] {
        let oilData:number[] = [];
        let gasData:number[] = [];
        let coalData:number[] = [];
        let elecData:number[] = [];
        ...
        let mySubset = files[2].filter((d: {SourceCountry: string, Type:string}) => {return getCountryName(d.SourceCountry) == country && d.Type == type;});
        ...
        let series:Worlddata.BarSeries[] = [];
        let oilSlice:Worlddata.BarSeries = {
            name: 'Oil',
            type: 'bar',
            data: oilData,
        ...
        let gasSlice:Worlddata.BarSeries = {
            name: 'Natural gas',
            type: 'bar',
            data: gasData,
            colorIndex: getColor.get('gas')
        };
        let coalSlice:Worlddata.BarSeries = {
            name: 'Coal',
            type: 'bar',
            data: coalData,
            colorIndex: getColor.get('coal')
        };
        let elecSlice:Worlddata.BarSeries = {
            name: 'Electricity',
            type: 'bar',
            data: elecData,
    ...
    function getTradeXAxis(type:string):string[] {
        ...
        let mySubset:any = files[2].filter((d: {SourceCountry:string, Type:string}) => {return getCountryName(d.SourceCountry) == country && d.Type == type;});
        mySubset.sort(function(a:any, b:any) {
            return  parseFloat(b.Total) - parseFloat(a.Total)});
        let Targetcountries:string[] = [...new Set(mySubset.map((d: {TargetCountry:string}) => d.TargetCountry))] as string[];
    ...
    function updateCountry():void {
        let element:Element = document.getElementsByClassName("dashboard-profile-country")[0].firstElementChild!;
        element.innerHTML = country;

        let importTitle:HTMLElement = document.getElementById('import_title')!;
        let exportTitle:HTMLElement = document.getElementById('export_title')!;
        let newImportTitle:string;
        let newExportTitle:string;
        if (country == "World") {
    ...
    function setSectorData():void {
        // Update sector data
        let newSectorData:Worlddata.SunburstData[] = getSectorData();
        ...
        sector_pie_chart.redraw();
         ...
         let errorSectors:HTMLElement = document.getElementById("energy_sectors_error")!;
    ...
    function setTradeData():void {
        // Update trade data
        let loopsImport:number = import_chart.series.length;
        let loopsExport:number = export_chart.series.length;
        let updatedImportData:Worlddata.BarSeries[] = getTradeData('Import');
        let updatedExportData:Worlddata.BarSeries[]  = getTradeData('Export');
        ...
        let xAxisCountriesImport:string[] = getTradeXAxis('Import');
        let xAxisCountriesExport:string[] = getTradeXAxis('Export');
        import_chart.update({
             xAxis: [{
    ...
    function setAreaData():void {
        // Update Area data
        let loops:number = area_chart.series.length;
        let updated_area_data:Worlddata.AreaSeries[] = get_area_data();
        // First empty the existing data
        ...
        area_chart.redraw();
         ...
         // Hide / show errors for missing data
         let errorArea:HTMLElement = document.getElementById("energy_area_error")!;
    ...
    function updateColorStops():void {
        ...
        if (source == "All") {
            updatedColorAxis = {
                ...
                stops: [
                    [0, getIndexColor(3)],
                    [0.1, getIndexColor(50)], // this is close to global average of 75
    ...
    function highlightCountry(deselect:boolean):void {
        let elements:NodeListOf<Element> = document.querySelectorAll(".energy-dashboard .highcharts-map-series .highcharts-point");
        if (deselect){
    ...
    var map_chart = Highcharts.mapChart({
        ...
        tooltip: {
            formatter: function (this: Worlddata.TooltipFormatterContextObject):string {
                ...
                var value:number = this.point.value!;
                var text = "";
                if (source == "All") {
        ...
        series: [{
            ...
            events: {
            click: function (e:Worlddata.ClickEvent) {
    ...
    let sector_pie_chart = Highcharts.chart({
        ...
        tooltip: {
            ...
            formatter: function (this: Worlddata.TooltipFormatterContextObject):string {
                ...
                let level1Points:Worlddata.PointOptions[] = this.series!.points!.filter(function (point:any){
                    return point.parent == "";
                });
                ...
                if ((this.point as any).parent == "") {
                    ...
                    drillText = "<br>and<b> " + value2 + "%</b> of all " + fLower((this.point as any).parent) + " energy use";
    ...
    let area_chart = Highcharts.chart({
        ...
        tooltip: {
            formatter: function (this: Worlddata.TooltipFormatterContextObject):string {
                ...
                let text1:string = '<b>' + value + "</b> petajoules of energy consumed came from <b>" + this.series!.name + '</b>';
                let perc:string = numberFormatter(this.y! / this.total! * 100);
                let text2:string = '<b>' + perc + "%</b> of all energy consumed in " + this.x + " came from <b>" + this.series!.name + '</b>';
        ...
        tooltip: {
            formatter: function (this: Worlddata.TooltipFormatterContextObject):string {
                ...
                if (country == "World") {
                    ...
                    text =  '<text><b>' + country + '</b> imported from <b>' + getCountryName(this.x as string) + '<br/>' +
                        numberFormatter(this.y!) + '</b> petajoules of <b>' + this.series!.name + '</b></text>';
                }
                return text;
            }
        ...
        tooltip: {
            formatter: function (this: Worlddata.TooltipFormatterContextObject):string {
                ...
                if (country == "World") {
                    ...
                    text =  '<text><b>' + country + '</b> exported to <b>' + getCountryName(this.x as string) + '<br/>' +
                        numberFormatter(this.y!) + '</b> petajoules of <b>' + this.series!.name + '</b></text>';
                }
                return text;
            }
    ...
    // Legends
    let sector_chart_points:Worlddata.PointOptions[] = sector_pie_chart.series[0].data;
    createLegend(sector_chart_points, sector_chart_points, '.energy-sector');
    let area_chart_points:Worlddata.AreaSeries[] = area_chart.series;
